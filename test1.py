...
url='http://10.124.161.2/cam-hi.jpg'
...
#defining global variables to keep track of iterations and to take an avarage for defining central point
iteration_count = 0
total_x = 0
total_y = 0
total_r = 0
...
def take_measure(threshold_img, threshold_ln, minLineLength, maxLineGap, diff1LowerBound, diff1UpperBound, diff2LowerBound, diff2UpperBound):
    ...
    global iteration_count, total_x, total_y, total_r
...
#for loop
    ...
    if circles is not None:
        a, b, c = circles.shape
        if iteration_count < 20: # Only for the first 5 iterations             
            x, y, r = avg_circles(circles, b)
            total_x += x
            total_y += y
            total_r += r
        ...
        for i in range(0,interval):
            for j in range(0,2):
                if (j%2==0):
                    p1[i][j] = total_x + 0.9 * r * np.cos(separation * i * 3.14 / 180) #point for lines
                else:
                    p1[i][j] = total_y + 0.9 * r * np.sin(separation * i * 3.14 / 180)
        text_offset_x = 10
        text_offset_y = 5
        for i in range(0, interval):
            for j in range(0, 2):
                if (j % 2 == 0):
                    p2[i][j] = total_x + r * np.cos(separation * i * 3.14 / 180)
                    p_text[i][j] = total_x - text_offset_x + 1.2 * r * np.cos((separation) * (i+9) * 3.14 / 180) #point for text labels, i+9 rotates the labels by 90 degrees
                else:
                    p2[i][j] = total_y + r * np.sin(separation * i * 3.14 / 180)
                    p_text[i][j] = total_y + text_offset_y + 1.2* r * np.sin((separation) * (i+9) * 3.14 / 180)  # point for text labels, i+9 rotates the labels by 90 degrees
        
        #Lines and labels
        for i in range(0,interval):
            cv2.line(img, (int(total_x), int(total_y)), (int(p2[i][0]), int(p2[i][1])), (0, 255, 255), 2)  # Using total_x and total_y as starting points
        ...
        final_line_list = []
        for i in range(0, len(lines)):#1300 that has 1300 x1,y1, x2,y2
            #2 itt
            for x1, y1, x2, y2 in lines[i]:
                diff1 = dist_2_pts(x, y, x1, y1)  # x, y is center of circle
                diff2 = dist_2_pts(x, y, x2, y2)  # x, y is center of circle
                # Its looking for the longest consistent line 
                if (diff1 > diff2):
                    temp = diff1
                    diff1 = diff2
                    diff2 = temp
        ...
        if (in_loop == 1):
            ...
            cv2.putText(img2, "Indicator OK!", (total_x, total_y), cv2.FONT_HERSHEY_SIMPLEX, 0.9,(0,255,0),2,cv2.LINE_AA)
